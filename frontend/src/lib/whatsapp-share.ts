...
export function getWhatsAppShareUrl(
  ...
  lines.push(``);
  lines.push(`Thank you for shopping with Hadi POS! 🙏`);
  ...
  if (window.electronAPI?.openWhatsAppShare) {
    return window.electronAPI.openWhatsAppShare(url)
      .then(async (result) => {
        ...
        return copyReceiptForWhatsApp(message);
      })
      .catch(() => copyReceiptForWhatsApp(message));
  }
  const popup = window.open('', '_blank');
  if (!popup) return copyReceiptForWhatsApp(message);
  popup.opener = null;
  popup.location.href = url;
  return Promise.resolve(true);
}

async function copyReceiptForWhatsApp(message: string): Promise<boolean> {
  try {
    await navigator.clipboard.writeText(message);
    toast.success('WhatsApp could not open. Bill copied — paste it into WhatsApp.');
    return true;
  } catch {
    return false;
  }
...
/** Opens an SMS composer with the paid-bill summary; the cashier sends it explicitly. */
export function shareBillViaSms(
  bill: Bill,
  ...
  if (!/^\+?[1-9]\d{5,14}$/.test(phone)) return Promise.resolve(false);
  const url = `sms:${phone}?body=${encodeURIComponent(getWhatsAppMessage(bill, tenant, opts, localeOverride))}`;
  if (window.electronAPI?.openSmsShare) {
    return window.electronAPI.openSmsShare(url)
