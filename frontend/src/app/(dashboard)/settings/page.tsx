...
export default function SettingsPage() {
  ...
  type BusinessForm = {
    businessName: string; businessType: 'restaurant' | 'fashion_retail'; countryCode: string; timezone: string; businessDayStartTime: string; currency: string;
  ...
  const [savedBusiness, setSavedBusiness] = useState<BusinessForm>({
    businessName: '', businessType: 'restaurant', countryCode: '', timezone: '', businessDayStartTime: '00:00', currency: '', billingType: 'postpaid',
    tablesRequired: true,
    taxRegistered: false,
  ...
  const resetBusiness = async () => {
    try {
      ...
      const loaded: BusinessForm = {
        ...
        businessType: d.business_type === 'fashion_retail' ? 'fashion_retail' : 'restaurant',
  ...
  const saveBusinessInfo = async (silent = false) => {
    ...
    try {
      ...
      updateCurrentTenant({ business_type: form.businessType, currency: form.currency, timezone: form.timezone, business_day_start_time: form.businessDayStartTime, country: form.countryCode, currency_display: form.currencyDisplay, number_digits: form.numberDigits, calendar: form.calendar });
        ...
        <div className="w-full md:w-40 md:min-w-[10rem] shrink-0 md:h-full md:min-h-0 md:flex md:flex-col">
          <div className="flex items-center gap-3 mb-6 shrink-0">
           ...
           <nav className="flex md:flex-col gap-0.5 overflow-x-auto md:flex-1 md:min-h-0 md:overflow-x-hidden md:overflow-y-auto md:overscroll-contain border-b md:border-b-0 md:border-e border-border pb-2 md:pb-0 md:pe-2">
            ...
            {!isRetail && <SettingsNavItem label={t('tabKds')} value="kds" active={activeTab} onClick={handleSettingsTabChange} />}
            {!isRetail && <SettingsNavItem label={t('tablesideOrdering')} value="server-app" active={activeTab} onClick={handleSettingsTabChange} />}
                ...
                <div>
                  ...
                  {isAdmin ? (
                    <select
                      ...
                      onChange={(e) => {
                        markHydrationTouched('businessType');
                        const businessType = e.target.value as 'restaurant' | 'fashion_retail';
                        setForm((p) => ({
                          ...p,
                          businessType,
                          billingType: businessType === 'fashion_retail' ? 'prepaid' : p.billingType,
                          tablesRequired: businessType === 'fashion_retail' ? false : p.tablesRequired,
                        }));
                      }}
                    ...
                    >
                      ...
                      <option value="fashion_retail">Fashion retail / clothing shop</option>
                    </select>
                  ) : (
                    <p className="font-medium text-foreground">{form.businessType === 'fashion_retail' ? 'Fashion retail / clothing shop' : 'Restaurant'}</p>
                  )}
                  <p className="mt-1 text-xs text-muted-foreground">Changing this does not delete your existing products or sales.</p>
                </div>
