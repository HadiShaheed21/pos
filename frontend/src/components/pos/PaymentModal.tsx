...
import { X, Wallet, ArrowLeftRight, CheckCircle2, Sparkles, User, Percent, Send, ChevronDown, MessageSquare } from 'lucide-react';
...
import { sendBillViaFlo, shareBillViaSms, shareBillViaWhatsApp } from '@/lib/whatsapp-share';
...
export default function PaymentModal({ bill, currency, onClose, onPaid, onBillUpdate }: Props) {
  ...
  // The cashier may send a receipt without creating a customer record first.
  const [receiptPhone, setReceiptPhone] = useState(() => cartCustomer?.phone || '7510645291');
  ...
  useEffect(() => {
    if (cartCustomer?.phone) setReceiptPhone(cartCustomer.phone);
  }, [cartCustomer?.phone]);
  ...
  const deliveryPhone = (() => {
    ...
    // A ten-digit Indian mobile number needs the country code for WhatsApp and SMS links.
    if ((currentTenant?.country || 'IN') === 'IN' && /^\d{10}$/.test(digits)) return `+91${digits}`;
    return receiptPhone.trim();
  })();
  ...
  const handleSendWhatsApp = async () => {
    ...
    setSendingWa(true);
    try {
      await sendBillViaFlo(bill, deliveryPhone, tenantForShare, whatsappSendT, { pointsEarned }, locale);
    } finally {
      setSendingWa(false);
    }
  };
  ...
  const handleShareWhatsApp = async () => {
    ...
    try {
      const opened = await shareBillViaWhatsApp(
        bill,
        { phone: deliveryPhone, country_code: cartCustomer?.country_code || 'IN' },
        tenantForShare,
        { pointsEarned },
        locale,
      );
  ...
  const handleShareSms = async () => {
    ...
    const opened = await shareBillViaSms(bill, { phone: deliveryPhone }, tenantForShare, { pointsEarned }, locale);
    if (!opened) toast.error('Could not open SMS for this customer number.');
  };
      ...
      <div className="bg-card w-full sm:max-w-4xl sm:max-h-[95vh] sm:flex sm:flex-col rounded-t-3xl sm:rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="px-5 pb-5 border-t border-border pt-3 space-y-2">
          <div className="rounded-xl border border-border bg-muted/30 px-3 py-2.5">
            <label htmlFor="receipt-delivery-phone" className="block text-sm font-medium text-foreground">
              Receipt delivery number
            </label>
            <div className="mt-1">
              <input
                id="receipt-delivery-phone"
                type="tel"
                inputMode="numeric"
                value={receiptPhone}
                ...
                aria-describedby="receipt-delivery-help"
              />
            </div>
            <p id="receipt-delivery-help" className="mt-1 text-xs text-muted-foreground">
              Indian 10-digit numbers use +91 automatically. WhatsApp and SMS options appear after payment; the bill is never sent automatically.
            </p>
          </div>
          {justPaid ? (
            <>
              <div className="grid grid-cols-2 gap-2">
                {isWhatsAppReady ? (
                  <Button onClick={handleSendWhatsApp} disabled={sendingWa || !deliveryPhone} className="bg-emerald-600 hover:bg-emerald-700" size="lg"><Send size={16} className="me-2" />{sendingWa ? t('processingPayment') : t('sendViaWhatsApp')}</Button>
                ) : (
                  <Button onClick={handleShareWhatsApp} disabled={!deliveryPhone} variant="outline" size="lg"><Send size={16} className="me-2" />WhatsApp</Button>
                )}
                <Button onClick={() => void handleShareSms()} disabled={!deliveryPhone} variant="outline" size="lg"><MessageSquare size={16} className="me-2" />SMS</Button>
              </div>
