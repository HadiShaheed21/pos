...
function ean13CheckDigit(value: string): string {
  let sum = 0;
...
function generateUniqueBarcode(db: ReturnType<typeof getDatabase>): string {
  // 290–299 is reserved for store-internal item numbers, making these suitable
  // for POS labels without claiming a GS1-issued company prefix.
  for (let attempt = 0; attempt < 100; attempt += 1) {
    const body = `290${crypto.randomInt(0, 1_000_000_000).toString().padStart(9, '0')}`;
    const barcode = `${body}${ean13CheckDigit(body)}`;
    const existing = db.prepare('SELECT 1 FROM products WHERE barcode = ? AND deleted_at IS NULL').get(barcode);
...
router.post('/barcode/generate', requireRole(...ROLE_ACCESS.ownerManager), (_req: Request, res: Response) => {
  try {
    res.json({ barcode: generateUniqueBarcode(getDatabase()) });
  } catch (error: any) {
    console.error('[API] Barcode generation failed:', error);
    res.status(500).json({ error: 'Could not generate a barcode' });
  }
});
