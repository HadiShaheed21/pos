...
const VALID_BUSINESS_TYPES = new Set(['restaurant', 'fashion_retail']);
...
function seedFashionRetail(db: ReturnType<typeof getDatabase>, profile: string): void {
  const categories = [
    ['cat-retail-men', 'Men', '#10284B', '👔', 1],
    ['cat-retail-women', 'Women', '#A855F7', '👗', 2],
    ['cat-retail-kids', 'Kids', '#F59E0B', '🧒', 3],
  ] as const;
  for (const [id, name, color, icon, sortOrder] of categories) insertCategory(db, id, name, color, icon, sortOrder);

  if (profile === 'empty') return;

  const products = [
    ['prod-retail-polo', 'cat-retail-men', 'Classic Cotton Polo', 'USN-POLO-NVY-M', '8901000000011', 1499, 720, 12],
    ['prod-retail-linen-shirt', 'cat-retail-men', 'Linen Resort Shirt', 'USN-LIN-SND-L', '8901000000012', 2199, 1080, 8],
    ['prod-retail-denim', 'cat-retail-women', 'Straight Fit Denim', 'USN-DEN-IND-30', '8901000000013', 2899, 1350, 7],
    ['prod-retail-midi-dress', 'cat-retail-women', 'Printed Midi Dress', 'USN-MIDI-RSE-M', '8901000000014', 3499, 1600, 9],
    ['prod-retail-graphic-tee', 'cat-retail-kids', 'Weekend Graphic Tee', 'USN-TEE-SKY-67', '8901000000015', 799, 310, 10],
  ] as const;

  const insert = db.prepare(`
    INSERT OR IGNORE INTO products (
      id, category_id, name, sku, barcode, price, cost, sort_order, is_active,
      track_inventory, stock_quantity, low_stock_threshold, created_at, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, 1, ?, 3, ?, ?)
  `);
  products.forEach(([id, categoryId, name, sku, barcode, price, cost, stock], index) => {
    insert.run(id, categoryId, name, sku, barcode, price, cost, index + 1, stock, now(), now());
  });
}
...
export function seedSetupProfile(
  ...
  if (businessType === 'fashion_retail') {
    seedFashionRetail(db, profile);
  ...
  try {
    ...
    db.transaction(() => {
      ...
      upsertSettings(db, {
        ...
        state_code,
        tax_registered,
        billing_type: billing_type || (normalizedBusinessType === 'fashion_retail' || normalizedServiceModel === 'qsr' ? 'prepaid' : 'postpaid'),
        tables_required: normalizedBusinessType === 'fashion_retail' ? 'false' : normalizedServiceModel === 'finedine' ? 'true' : 'false',
      ...
      seedSetupProfile(db, normalizedSetupProfile, normalizedServiceModel, language, country, normalizedBusinessType);
    })();
