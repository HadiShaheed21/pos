...
function businessShape(s: Record<string, string>) {
  return {
    ...
    business_type: s.business_type === 'fashion_retail' ? 'fashion_retail' : 'restaurant',
...
router.put('/business', requireRole(...ROLE_ACCESS.ownerManager), (req: Request, res: Response) => {
  try {
    ...
    const normalizedBusinessType = business_type === undefined
      ? undefined
      : String(business_type).trim();

    if (normalizedBusinessType !== undefined && !['restaurant', 'fashion_retail'].includes(normalizedBusinessType)) {
    ...
    upsertSettings(db, {
      business_name, business_type: normalizedBusinessType, timezone,
