...
import { isSafeSmsShareUrl, isSafeWhatsAppShareUrl } from './security/url-allowlist';
...
export function registerIpcHandlers(
  ...
  handle('whatsapp-open-share', async (_event, rawUrl: unknown) => {
    ...
    try {
      ...
      // macOS can reject a browser hand-off even when the WhatsApp desktop app is installed.
      // Retry through its native URL scheme using the same validated phone number and message.
      if (process.platform === 'darwin') {
        try {
          const shareUrl = new URL(rawUrl);
          const phone = shareUrl.pathname.slice(1);
          const text = shareUrl.searchParams.get('text') || '';
          await shell.openExternal(`whatsapp://send?phone=${phone}&text=${encodeURIComponent(text)}`);
          return { success: true };
        } catch (fallbackError: unknown) {
          console.error('[IPC] WhatsApp share open failed:', sanitizeLogText(fallbackError));
        }
      } else {
  ...
  handle('sms-open-share', async (_event, rawUrl: unknown) => {
    if (typeof rawUrl !== 'string' || !isSafeSmsShareUrl(rawUrl)) {
      return { success: false, error: 'Invalid SMS share URL' };
    }
    try {
      await shell.openExternal(rawUrl);
      return { success: true };
    } catch (error: unknown) {
      console.error('[IPC] SMS share open failed:', sanitizeLogText(error));
